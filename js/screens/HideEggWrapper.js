...
export default class HideEggWrapper extends Component {
    ...
    requestLocatePermission().then(() => {
      Geolocation.getCurrentPosition(
        (position) => {
          ...
          this.props.updateBunnyPosition(
            this.props.unplacedEggs[0],
            position.coords,
          );
        },
  ...
  render() {
        ...
        <ViroARScene>
          ...
          <ViroNode position={[0, 0, 0]}>
            <ViroSphere
              ...
              onDrag={(dragToPos) =>
                this.props.onHideEgg(this.props.unplacedEggs[0], dragToPos)
              }
              onClick={() =>
    ...
    if (this.props.unplacedEggs.length > 0) {
      return <ViroARSceneNavigator initialScene={{scene: EggPlacer}} />;
    } else {
